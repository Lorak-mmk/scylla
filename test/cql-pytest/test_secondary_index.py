...
# Reproducer for issue #3404: A column name may contain all sorts of
...
def test_index_weird_chars_in_col_name(cql, test_keyspace):
    with tempfile.TemporaryDirectory() as tmpdir:
        # When issue #3404 exists, column name ../../...../tmpdir/xyz will
        ...
        with new_test_table(cql, test_keyspace, schema) as table:
            ...
            # something in tmpdir! If it has, we have issue #3404.
            assert os.listdir(tmpdir) == []
            # Check that the expected index name was chosen - based on
            # only the alphanumeric characters of the column name.
            ks, cf = table.split('.')
            ...
            cleaned_up_column_name = ''.join(filter(str.isalnum, magic_path))
