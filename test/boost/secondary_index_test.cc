...
SEASTAR_TEST_CASE(test_secondary_index_collections) {
    return do_with_cql_env_thread([] (cql_test_env& e) {
        ...
        auto non_frozen = message_contains("with non-frozen collection type");
        ...
        eventually([&] {
            auto res = e.execute_cql("SELECT p from t where m1 CONTAINS 'three'").get0();
            assert_that(res).is_rows().with_rows_ignore_order({{{{int32_type->decompose(2)}}}, {{{int32_type->decompose(3)}}}});
        ...
        eventually([&] {
            auto res = e.execute_cql("SELECT p from t where m1 CONTAINS KEY 3").get0();
            assert_that(res).is_rows().with_rows_ignore_order({{{{int32_type->decompose(2)}}}, {{{int32_type->decompose(3)}}}});
        ...
        eventually([&] {
            auto res = e.execute_cql("SELECT p from t where m1[3] = 'three'").get0();
        ...
        eventually([&] {
            auto res = e.execute_cql("SELECT p from t where l1 CONTAINS 2").get0();
